...
## GitHub 저장소(`co1324/exam`)에 코드 올리기

아래 절차를 따르면 현재 로컬에 있는 프로젝트를 사용자의 GitHub 저장소 `https://github.com/co1324/exam.git`에 업로드할 수 있습니다. 이미 Git을 어느 정도 사용해 보았다면 필요한 단계만 골라 수행해도 괜찮습니다.

1. **GitHub 저장소 준비**
   - GitHub 계정으로 로그인한 뒤, 브라우저에서 `https://github.com/co1324/exam.git` 주소가 정상적으로 접근되는지 확인합니다.
   - 아직 저장소가 비어 있다면, GitHub 웹 UI에서 `Code` > `Use GitHub CLI`/`HTTPS` 영역의 주소를 복사해 둡니다.

2. **로컬 저장소 초기화 또는 기존 저장소 사용**
   - 이미 본 프로젝트를 클론했다면 이 단계는 건너뛰어도 됩니다.
   - 새로 시작한다면 아래 명령으로 프로젝트 폴더를 Git 저장소로 초기화합니다.
     ```bash
     cd /path/to/exam-proofreading
     git init
     git add .
     git commit -m "초기 커밋"
     ```
...
3. **GitHub 원격 저장소 연결**
   - 원격 이름을 `origin`으로 지정하고 GitHub 저장소 URL을 연결합니다. 기존에 다른 원격이 있으면 `git remote remove origin`으로 삭제한 뒤 다시 추가하세요.
     ```bash
     git remote add origin https://github.com/co1324/exam.git
     ```

4. **GitHub Personal Access Token(필요 시)**
   - HTTPS 방식으로 푸시할 때 비밀번호 대신 토큰이 필요할 수 있습니다. [GitHub Settings > Developer settings > Personal access tokens](https://github.com/settings/tokens)에서 `repo` 권한을 가진 토큰을 생성하세요.
   - 토큰은 한 번만 표시되므로 안전한 곳에 보관한 뒤, 푸시 시 비밀번호 대신 입력합니다.

5. **코드 푸시**
   - 로컬 브랜치가 `main`이라면 아래 명령으로 GitHub에 업로드할 수 있습니다.
     ```bash
     git push -u origin main
     ```
   - 다른 브랜치를 사용 중이라면 `main` 대신 브랜치 이름을 입력하면 됩니다.

6. **업로드 확인**
   - 브라우저에서 `https://github.com/co1324/exam` 페이지를 열어 파일이 정상적으로 올라갔는지 확인합니다.
   - 이후 변경 사항이 생길 때마다 `git add`, `git commit`, `git push` 순서로 반복하면 됩니다.

> **TIP**: 푸시할 때 `fatal: remote origin already exists` 오류가 나온다면 `git remote set-url origin https://github.com/co1324/exam.git` 명령으로 URL만 수정하면 됩니다.
